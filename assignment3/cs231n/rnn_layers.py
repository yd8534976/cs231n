...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = tanh_forward(a)
    cache = (a, x, prev_h, Wx, Wh)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    a, x, prev_h, Wx, Wh = cache
    dout = tanh_backward(dnext_h, a)
    dx = dout.dot(Wx.T)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    g = tanh_forward(ag)
    next_c = f * prev_c + i * g
    h = tanh_forward(next_c)
    next_h = o * h
    
    cache = (x, prev_h, prev_c, Wx, Wh, b, a, ai, af, ao, ag, i, f, o, g, next_c, h)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    x, prev_h, prev_c, Wx, Wh, b, a, ai, af, ao, ag, i, f, o, g, next_c, h = cache
    _, H = prev_h.shape
    
    do = dnext_h * h
    dh = dnext_h * o
    dnext_c += tanh_backward(dh, next_c)
    dprev_c = dnext_c * f
    df = dnext_c * prev_c
    di = dnext_c * g
    dg = dnext_c * i
...
def lstm_backward(dh, cache):
    ...
    N, T, _ = dh.shape
    dprev_c = 0
    dx_last, dprev_h, dprev_c, dWx, dWh, db = lstm_step_backward(dh[:, T-1, :], dprev_c, cache[T-1])
    D, _ = dWx.shape
    dx = np.zeros((N, T, D))
    dx[:, T-1, :] = dx_last
    for i in range(2, T + 1):
